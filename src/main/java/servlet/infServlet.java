...
@WebServlet(name = "infServlet", value = "/inf1")
...
//            ArrayList<Usedateimg> uselist = database.getAlluserimg(uId);//抓取到数据
            ArrayList<infInfo> uselist = database.getinfInfo(storedItemId);
            request.setAttribute("userInfo",uselist);
            // 转发到显示用户信息的JSP页面
            request.getRequestDispatcher("information1.jsp").forward(request, response);
            //转发
